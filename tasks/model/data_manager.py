...
from tasks import session, validator


class DataManager:

    def get_task_values(self, option: str) -> List[List]:
        """
        ...
        return [[task.name,
                 ...
                 task.task_hash]
                for task in self.get_tasks(option)]

    def get_table(self, option: str) -> List[List]:
        """
        ...
        :return: list containing lists of column names and tasks values with an option selected, prepared to be printed
                    for the user.
        """
        return [self.get_column_names(), *self.get_task_values(option)] if self.get_task_values(option) \
            else [self.get_column_names(), table_for_not_found_data]

    def get_table_with_one_record(self, task: Task):
        """
...
class CMDDataManager(DataManager):

    def validate_and_add_task(self, sys_args: List[str]) -> Tuple[bool, int]:
        """
        ...
        """
        try:
            task_validation = validator.validate_add_task_arguments(sys_args)
            if type(task_validation) == dict:
                new_task = self.add_task(name=task_validation.get("name"),
    ...
    def find_task_for_table(self, sys_args: List[str]) -> Union[Tuple[bool, List], Tuple[bool, int]]:
        """
        Function returning True or False value, indicating its success and tables with the found "Task" object or
        message index in case of its failure. Used for listing one record for rind or remove tasks functionality.
        ...
        """
        hash_index = 2
        task = self.find_task_by_hash(sys_args, hash_index)
        if task:
            return True, self.get_table_with_one_record(task)
        return False, 5

    def finish_task(self, sys_args: List[str]) -> Tuple[bool, int]:
        """
    ...
    def undo_task(self, sys_args: List[str]) -> Tuple[bool, int]:
        """
        ...
        :return: tuple with boolean value meaning if it was successful and index of message
        """
        task = self.find_task_by_hash(sys_args, 2)
    ...
    def validate_and_update_task(self, sys_args: List[str]) -> Tuple[bool, int]:
        """
        A function that validates entered system arguments for updating a job and changes or removes them depending
        ...
        try:
            task = self.find_task_by_hash(sys_args, -1)
            if not task:
                return False, 5
            new_values_validator = validator.validate_update_args(sys_args)
    ...
    def validate_and_get_list(self, sys_args: List[str]) -> Union[Tuple[bool, List], Tuple[bool, int]]:
        """
        ...
        :return: tuple with boolean value meaning if it was successful and table, if validated, or wrong option which
                was provided
        """
        list_option = validator.validate_listing_arguments(sys_args)
        if type(list_option) != str:
            return False, list_option
        return True, self.get_table(list_option)

    def validate_and_remove_task(self, sys_args: List[str]) -> Tuple[bool, int]:
        """
...
class LazyDataManager(DataManager):

    def lazy_find_and_remove_task(self, task_hash: str) -> Tuple[bool, int]:
        """
        ...
        :return: tuple with boolean value meaning if it was successful and index of message
        """
        task = self.lazy_find_task(task_hash)
        if task:
            session.delete(task)
            session.commit()
            return True, 4
    ...
    def lazy_update_task(self, task: Task, name: str, deadline: datetime.date, description: str) -> Tuple[bool, int]:
        """
        ...
        :return: tuple with boolean value meaning if it was successful and index of message
        """
        if not self.lazy_find_task(task.task_hash):
            return False, 5
    ...
    def lazy_finish_or_undo_task(self, task: Task) -> Tuple[bool, int]:
        """

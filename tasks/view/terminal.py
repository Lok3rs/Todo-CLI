...
class View:
    ...
    def print_message(self, success: bool, msg_index: Union[int, List], print_help: bool = False,
                      listing: bool = False):
        if success and listing:
            self.print_table(msg_index)
    ...
    def print_table(self, tables: List[list]):
        try:
            table_with_strings = [[str(element) for element in table] for table in tables]

            max_column_width = self.get_max_column_width(table_with_strings)
            break_line, start_line, end_line = self.get_borders(max_column_width)

            print(start_line)
            for row in table_with_strings:
                self.print_row(row, max_column_width)
